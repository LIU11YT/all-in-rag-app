...
class GenerationIntegrationModule:
    ...
    def setup_llm(self):
        ...
        self.llm = ChatOpenAI(
            model=self.model_name,   # 例如 deepseek-chat
            temperature=self.temperature,
            max_tokens=self.max_tokens,
            api_key=api_key,
    ...
    def generate_basic_answer(self, 
                            query: str, 
        ...
        meta_context = self._compose_meta_context(
...
请注意：
1. 短期记忆优先用于解析“这个、那个、第二个、上一个、刚才推荐的”等最近指代
2. 长期记忆优先用于遵循用户长期偏好，如口味、忌口、饮食目标、回答风格
3. 如果短期记忆和长期记忆冲突，优先参考短期记忆中的当前任务上下文
4. 如果上下文不足，请诚实说明，不要编造
...
回答:""")
        ...
        # 使用LCEL构建链
        chain = (
            {"question": RunnablePassthrough(), "context": lambda _: context, "meta_context": lambda _: meta_context}
            | prompt
            | self.llm
            | StrOutputParser()
        )
...
你的任务：
1. 判断该查询是否已经足够明确，可以直接用于食谱搜索
2. 如果查询依赖历史对话中的上下文，请先结合历史信息解析指代
3. 只有在“当前查询 + 历史信息”仍不足以确定明确搜索意图时，才进行重写
...
二、上下文依赖但可解析的查询（先解析，再返回解析后的查询）
如果查询中包含指代、编号、承接表达，不要直接判定为模糊，而应优先结合历史对话解析：
- 顺序指代：如“第一个”“第二个”“上一个”“前一个”
- 指示代词：如“这个”“那个”“这个菜”“那个菜”
- 时间承接：如“刚刚那个”“刚才推荐的”“上次说的”
- 任务承接：如“它怎么做”“这个需要什么食材”“换一个类似的”

处理原则：
- 如果历史对话中存在最近的菜品列表、当前讨论菜品、最近推荐结果，则优先解析
- 例如：
  - 若历史中最近推荐了 [宫保鸡丁, 西红柿炒蛋, 鱼香肉丝]
  - 用户问“第二个怎么做”
  - 应解析为“西红柿炒蛋怎么做”
- 若用户问“上一个需要什么食材”
  - 应解析为最近上下文中上一个被提到的具体菜品 + “需要什么食材”

三、真正模糊不清的查询（需要重写）
只有以下情况才需要重写：
- 过于宽泛：如“做菜”“有什么好吃的”“推荐个菜”
- 缺乏主体：如“川菜”“素菜”“简单的”
- 口语化但无明确上下文：如“想吃点什么”“有饮品推荐吗”
- 包含指代表达，但历史对话中无法唯一解析：如“上一个”“那个”“换一个”，但没有明确参照对象

输出要求：
- 如果查询直接明确，输出原查询
- 如果查询依赖历史但可以解析，输出“解析后的明确查询”
- 如果查询确实模糊，输出“更适合搜索的重写查询”
- 不要输出解释，只输出最终查询文本
...
回答:""")
    ...
    def _compose_meta_context(
        ...
        if short_memory_context:
            parts.append(
                "短期记忆（优先用于解析最近指代、最近推荐顺序、当前讨论对象）:\n"
                f"{short_memory_context}"
            )
        if long_memory_context:
            parts.append(
                "长期记忆（优先用于个性化回答，如用户口味、限制、风格偏好）:\n"
